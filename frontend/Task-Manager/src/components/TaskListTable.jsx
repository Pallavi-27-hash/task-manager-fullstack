...
const TaskListTable = ({tableData}) => {
  const getStatusBadgeColor = (status) => {
    switch (status) {
      ...
      case 'InProgress':
        return 'bg-cyan-100 text-cyan-100 border border-cyan-100';
      case 'Completed':
  ...
  const getPriorityBadgeColor = (priority) => {
    switch (priority) {
      ...
      case 'Medium':
        return 'bg-orange-100 text-yellow-500 border border-orange-200';
      case 'Low':
        return 'bg-green-100 text-green-500 border border-green-200';
      default:
        return 'bg-gray-100 text-gray-500 border border-gray-200';
          ...
          {tableData.map((task) => (
            <tr key={task._id} className="border-t  border-gray-200">
              ...
              <td className="py-4 px-4 text-gray-700 text-[13px] text-nowrap hidden md:table-cell">
                {task.createdAt ? moment(task.createdAt).format('DD MMM YYYY') : 'N/A'} </td>
